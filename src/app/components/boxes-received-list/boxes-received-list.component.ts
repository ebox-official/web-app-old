...
export class BoxesReceivedListComponent implements OnInit, OnDestroy {
    ...
    private mapBoxes(boxes) {
        ...
        // This mapping belongs to ContractService, but it's done here for the time being. It performant to map only those boxes that are received
        this.fetchedBoxes = boxes
            .reduce((results, box, index) => {

                let boxOfAccount;
                if (box.recipient == this.contractServ.selectedAccount$.getValue()) {
                    boxOfAccount = true;
                }
